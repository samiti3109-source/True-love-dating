...
- For production use: change `SECRET_KEY` in `app.py` to a real secret, swap
  the `sqlite3` calls in `db.py` for a Postgres/MySQL driver if you outgrow
  SQLite, and run behind a real WSGI server (gunicorn, etc.) instead of
  `app.run(debug=True)`.
- The app only depends on Flask itself — the database uses Python's built-in
  `sqlite3` module, so there's no separate ORM or database server to install.
